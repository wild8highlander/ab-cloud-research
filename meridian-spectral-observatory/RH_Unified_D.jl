...
const _MR_BRAND = (title = "RH·UNIFIED", version = "1.0",
                   subtitle = "AB-cloud ↔ ζ-zeros correlation core · 500 verified zeros")
...
_mr_banner("RH·UNIFIED", "1.0",
           "AB-cloud ↔ ζ-zeros correlation core",
           "$N_ZETA_ZEROS_AVAILABLE verified ζ-zeros (mpmath.zetazero, Odlyzko-verified)",
           "⟨r⟩ = 0.6162 ≈ R_GUE = 0.5996 · 61 magic α · vortices 2–100+")
_mr_section("запуск / run")
println("  " * _mr_c("cfg = preset_quick();       run(cfg)", "bold") * _mr_c("   — быстрый тест", "dim"))
println("  " * _mr_c("cfg = preset_full();        run(cfg)", "bold") * _mr_c("   — полный анализ", "dim"))
println("  " * _mr_c("cfg = preset_magic();       run(cfg)", "bold") * _mr_c("   — sweep 61 магических α", "dim"))
println("  " * _mr_c("cfg = preset_deep_sweep();  run(cfg)", "bold") * _mr_c("   — sweep α от -16 до +16", "dim"))
println("  " * _mr_c("cfg = preset_many_vortices(12); run(cfg)", "bold") * _mr_c(" — 12 вихрей", "dim"))
println("  " * _mr_c("cfg = preset_ring_vortices(8);  run(cfg)", "bold") * _mr_c(" — кольцо из 8 вихрей", "dim"))
_mr_section("helpers")
println("  " * _mr_c("list_presets()", "bold") * _mr_c("    — список всех пресетов", "dim"))
println("  " * _mr_c("show_config(cfg)", "bold") * _mr_c("  — показать параметры cfg", "dim"))
println("  " * _mr_c("help_dynamic()", "bold") * _mr_c("    — подробная справка", "dim"))
println()
