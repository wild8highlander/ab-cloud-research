...
#   ✓ ζ-нули: 500 встроенных (fallback) + автозагрузка до 3000 из внешнего
#     файла zeta_zeros_50000.txt (источник: Odlyzko zeta_tables), если он лежит рядом (см. _load_zeta_zeros)
...
const ZETA_ZEROS_EMBEDDED_COUNT = 500   # надёжный fallback, без внешних файлов

const ZETA_ZEROS_EMBEDDED = Float64[
...
Пытается расширить список ζ-нулей внешним файлом `zeta_zeros_50000.txt`
(один Float64 на строку, n-я строка = n-й ноль), который кладётся РЯДОМ
с этим .jl. Если файла нет — используются 500 встроенных (надёжный
fallback, ноль внешних зависимостей, как и раньше).

САМОПРОВЕРКА: первые ZETA_ZEROS_EMBEDDED_COUNT нулей из файла должны
совпадать со встроенными (|Δ|<1e-6) и быть монотонны — иначе файл
игнорируется целиком (лучше 500 верных, чем N с молчаливой порчей
данных). Почему это важно: с 500 нулями K(t) считается на чанках по
~250 (zeta_half = N÷2), что даёт огромный sampling-шум — несглаженный
K(ζ) на чанке из 250 подскакивает до ~3.5 при t=1 (должно быть ≈1.0),
чисто статистический артефакт малой выборки, не свойство AB-облака.
3000 нулей дают chunk до 1500, сравнимый с размером AB-стороны.
"""
function _load_zeta_zeros()
    for path in (joinpath(@__DIR__, "zeta_zeros_50000.txt"),
                 joinpath(@__DIR__, "zeta_zeros_3000.txt"),
                 joinpath(@__DIR__, "zeta_zeros_extra.txt"),
        ...
        try
            ...
            if length(vals) <= ZETA_ZEROS_EMBEDDED_COUNT
                println("⚠️  $path: $(length(vals)) нулей (<= встроенных $ZETA_ZEROS_EMBEDDED_COUNT) — пропущен.")
                continue
            end
            if !issorted(vals)
                println("⚠️  $path: нули не монотонны — файл повреждён, пропущен.")
                continue
            end
            n_check = ZETA_ZEROS_EMBEDDED_COUNT
            max_dev = maximum(abs.(vals[1:n_check] .- ZETA_ZEROS_EMBEDDED[1:n_check]))
            if max_dev > 1e-6
                println("⚠️  $path: первые $n_check нулей расходятся со встроенными (max|Δ|=$max_dev) — пропущен.")
                continue
            end
            println("✓ Загружено $(length(vals)) ζ-нулей из $path (проверено по первым $n_check встроенным, max|Δ|=$max_dev).")
            return vals
        catch e
            println("⚠️  Не удалось прочитать $path ($e) — используются встроенные 500.")
            continue
        end
    end
    return ZETA_ZEROS_EMBEDDED
end

const ZETA_ZEROS = _load_zeta_zeros()
const N_ZETA_ZEROS_AVAILABLE = length(ZETA_ZEROS)
const ZETA_ZEROS_VERIFIED_COUNT = N_ZETA_ZEROS_AVAILABLE

"""
    _load_high_T_blocks()

Грузит дополнительные блоки нулей ζ(s) с очень больших высот T
(индексы ~10^12, ~10^21, ~10^22), если рядом лежит
`zeta_zeros_highT_blocks.txt`. Формат: секции
`# BLOCK index=<label> offset=<int>`, далее значения gamma−offset
(локальные дельты; абсолютный offset не нужен — ⟨r⟩ инвариантен к
сдвигу, а складывать offset~10^21 с дельтой~10^2 в Float64 всё равно
невозможно: не хватает значащих цифр). Возвращает пустой Dict, если
файла нет (фича опциональна, без него всё работает как раньше).
"""
function _load_high_T_blocks()
    blocks = Dict{String, Vector{Float64}}()
    for path in (joinpath(@__DIR__, "zeta_zeros_highT_blocks.txt"),
        ...
        try
            ...
            for line in eachline(path)
                ...
                if startswith(s, "#")
                    m = match(r"index=(\S+)", s)
                    current = m === nothing ? s : m.captures[1]
                    blocks[current] = Float64[]
                elseif !isempty(current)
                    push!(blocks[current], parse(Float64, s))
                end
            end
            !isempty(blocks) && return blocks
        catch e
            println("⚠️  Не удалось прочитать $path ($e).")
        end
    end
    return blocks
end

const ZETA_HIGH_T_BLOCKS = _load_high_T_blocks()

"""
    verify_high_T_universality()

Доп. проверка: GUE-универсальность (⟨r⟩ ≈ R_GUE = 0.5996) должна
выполняться не только для первых ~10^4-10^5 нулей ζ(s) (низкий T,
основной массив ZETA_ZEROS), но и сколь угодно высоко по критической
прямой — прямое следствие гипотезы Монтгомери о парных корреляциях.
Используя блоки на индексах ~10^12, 10^21, 10^22, считаем ⟨r⟩
независимо в каждом и сравниваем с R_GUE — если оно держится на всех
масштабах, это куда более сильное свидетельство универсальности, чем
совпадение только у первых 500-50000 нулей (которое теоретически могло
быть особенностью малых T).
"""
function verify_high_T_universality()
    if isempty(ZETA_HIGH_T_BLOCKS)
        println("ℹ️  zeta_zeros_highT_blocks.txt не найден — high-T проверка пропущена.")
        return nothing
    end
    println("\n" * "─"^78)
    println("  GUE-УНИВЕРСАЛЬНОСТЬ НА БОЛЬШИХ ВЫСОТАХ T")
    println("─"^78)
    results = Dict{String, Float64}()
    for (label, deltas) in sort(collect(ZETA_HIGH_T_BLOCKS), by=first)
        sr = spacing_ratios(deltas)
        r = isempty(sr) ? NaN : mean(sr)
        results[label] = r
        dev = abs(r - R_GUE)
        status = dev < 0.01 ? "✓" : (dev < 0.03 ? "~" : "✗")
        println("  индекс зеро ~$label : n=$(length(deltas))  ⟨r⟩=$(round(r,digits=4))  " *
                "|Δ от R_GUE|=$(round(dev,digits=4))  $status")
    end
    println("  (для сравнения, низкий T, n=$(length(ZETA_ZEROS)): ⟨r⟩=$(round(mean_r(ZETA_ZEROS),digits=4)))")
    println("─"^78)
    return results
end
...
struct MetricsResult
    ...
    p_rms_GUE::Float64
    p_mean::Float64
    r_byte_quantized::Float64
    byte_quant_diff::Float64
    byte_quant_robust::Bool
    rng_entropy_bits::Float64
    rng_chi2_z::Float64
    rng_uniform_ok::Bool
    byte_match::Bool
...
"""
    compute_byte_level_diagnostics(eigs_ab, r_ab; n_bytes=200_000, seed=12345)

Тест 8: "вариативность по байтам" — две независимые проверки того, что
GUE-результат не артефакт представления чисел/генератора случайности.

  (a) BYTE-QUANTIZATION ROBUSTNESS. Линейно сжимаем собственные значения
      AB-облака в 8-битный диапазон [0,255], округляем до целого (как
      если бы хранили их в одном байте), растягиваем обратно и пересчи-
      тываем ⟨r⟩ из этого огрублённого (256 уровней) представления.
      Если ⟨r⟩ выживает такое огрубление (расхождение < byte_tol),
      GUE-классификация — не хрупкий артефакт float-точности, а грубое
      статистическое свойство, видимое даже при 256 уровнях.

  (b) RNG BYTE-ENTROPY / UNIFORMITY. Берём n_bytes байт напрямую из
      MersenneTwister той же природы, что используется для джиттера
      вихрей и диагонального беспорядка (rand(rng, UInt8)), считаем
      энтропию Шеннона гистограммы (идеал = 8.0 бит) и χ²-статистику
      на равномерность по 256 ячейкам (сравниваем с df=255 через
      нормальную аппроксимацию большого df — без зависимости от
      SpecialFunctions). Смещённый/маловыходный RNG тихо вносил бы
      структуру в КАЖДУЮ конфигурацию вихрей в этом файле.
"""
function compute_byte_level_diagnostics(eigs_ab::AbstractVector{<:Real}, r_ab::Real;
                                          n_bytes::Int=200_000, seed::Int=12345,
                                          byte_tol::Float64=0.02)
    # (a) огрубление до байта (256 уровней) и пересчёт <r>
    lo, hi = minimum(eigs_ab), maximum(eigs_ab)
    span = hi - lo
    r_quantized = NaN
    byte_quant_diff = NaN
    if span > 0
        levels = clamp.(round.(Int, (eigs_ab .- lo) ./ span .* 255), 0, 255)
        quantized = lo .+ (levels ./ 255) .* span
        sr_q = spacing_ratios(quantized)
        if !isempty(sr_q)
            r_quantized = mean(sr_q)
            byte_quant_diff = abs(r_quantized - r_ab)
        end
    end
    byte_quant_robust = !isnan(byte_quant_diff) && byte_quant_diff < byte_tol

    # (b) энтропия и равномерность RNG на уровне байт
    rng = MersenneTwister(seed)
    bytes = rand(rng, UInt8, n_bytes)
    counts = zeros(Int, 256)
    @inbounds for b in bytes
        counts[Int(b) + 1] += 1
    end
    probs = counts ./ n_bytes
    shannon_entropy = -sum(p > 0 ? p * log2(p) : 0.0 for p in probs)
    expected = n_bytes / 256
    chi2_stat = sum((counts .- expected) .^ 2 ./ expected)
    df = 255
    chi2_z = (chi2_stat - df) / sqrt(2.0 * df)   # CLT-аппроксимация для большого df
    rng_uniform_ok = abs(chi2_z) < 3.0
    entropy_ok = shannon_entropy > 7.9

    byte_match = byte_quant_robust && rng_uniform_ok && entropy_ok

    return (r_byte_quantized=r_quantized, byte_quant_diff=byte_quant_diff,
            byte_quant_robust=byte_quant_robust, rng_entropy_bits=shannon_entropy,
            rng_chi2_z=chi2_z, rng_uniform_ok=rng_uniform_ok, byte_match=byte_match)
end
...
function compute_all_metrics(eigs_ab::AbstractVector{<:Real},
    ...
    # K(t)
    # ВАЖНО: K(AB) и K(ζ) должны считаться с ОДИНАКОВЫМ chunk size.
    # При разном N у них разная амплитуда осцилляций → корреляция занижается.
    # Решение: используем минимальный chunk size из (eigs_per_seed, zeta_chunk)
    # для ОБОИХ спектров, чтобы они были максимально симметричны.
    t_values = collect(0.0:0.02:2.0)
    # Размер chunk: минимум из одного seed AB и половины ζ-нулей
    zeta_half = length(zeta_zeros) ÷ 2
    ab_seed = eigs_per_seed > 0 ? eigs_per_seed : length(eigs_ab)
    # Используем ОДИНАКОВЫЙ chunk size для обоих (минимум из двух)
    # Но не менее 100 (иначе слишком шумно)
    common_chunk = max(100, min(ab_seed, zeta_half))
    K_ab = compute_K_empirical(eigs_ab, t_values; method=:polynomial, smooth=true,
                                smooth_window=3,
                                eigs_per_seed=common_chunk)
    ...
    if cfg_verbose_metrics
        println("    [metrics] K(t): common_chunk=$common_chunk, ab_seed=$ab_seed, zeta_half=$zeta_half")
        # Показываем K в нескольких ключевых точках: t=0.1, 0.5, 1.0, 1.5
    ...
    # ИСПРАВЛЕНО: раньше K_match_gue фактически проверял K_corr_zeta>0.8
    # (баг именования — "GUE-тест" под капотом был более жёстким
    # zeta-тестом), а честно вычисленный K_match_zeta (>0.7) существовал
    # как мёртвый код — не входил ни в tests_passed, ни в вывод вердикта.
    # Теперь это два РАЗНЫХ теста с понятным смыслом:
    K_match_gue = K_corr_gue > 0.8        # тест 3: K(t) похож на ТЕОРИЮ GUE
    K_match_zeta = K_corr_zeta > 0.7       # тест 7: K(t) коррелирует с РЕАЛЬНЫМИ ζ-нулями
    if K_match_gue
        tests_passed += 1
    end
    if K_match_zeta
    ...
    # Байтовая диагностика (тест 8): огрубление <r> до 256 уровней +
    # энтропия/равномерность RNG на уровне байт (см. docstring выше).
    byte_diag = compute_byte_level_diagnostics(eigs_ab, r_ab)
    if cfg_verbose_metrics
        println("    [metrics] byte: r_full=$(round(r_ab,digits=4)) r_byte256=$(round(byte_diag.r_byte_quantized,digits=4)) " *
                "Δ=$(round(byte_diag.byte_quant_diff,digits=4)) | RNG entropy=$(round(byte_diag.rng_entropy_bits,digits=4))bit " *
                "χ²z=$(round(byte_diag.rng_chi2_z,digits=2))")
    end
    if byte_diag.byte_match
        tests_passed += 1
    end
    ...
    return MetricsResult(
        ...
        byte_diag.r_byte_quantized, byte_diag.byte_quant_diff, byte_diag.byte_quant_robust,
        byte_diag.rng_entropy_bits, byte_diag.rng_chi2_z, byte_diag.rng_uniform_ok, byte_diag.byte_match,
...
function save_csv_report(result::MetricsResult, all_results::Vector{MetricsResult}, filepath::String)
    open(filepath, "w") do f
        ...
        headers = ["label", "L", "alpha", "W", "sigma", "n_eigs",
                   ...
                   "r_byte_quantized", "byte_quant_diff", "byte_quant_robust",
                   "rng_entropy_bits", "rng_chi2_z", "rng_uniform_ok", "byte_match",
                   "composite_score", "tests_passed", "tests_total"]
        ...
        for r in all_results
            row = [
                ...
                round(r.r_byte_quantized, digits=6), round(r.byte_quant_diff, digits=6),
                r.byte_quant_robust ? "TRUE" : "FALSE",
                round(r.rng_entropy_bits, digits=6), round(r.rng_chi2_z, digits=4),
                r.rng_uniform_ok ? "TRUE" : "FALSE", r.byte_match ? "TRUE" : "FALSE",
...
function save_txt_report(result::MetricsResult, all_results::Vector{MetricsResult},
    ...
    open(filepath, "w") do f
        ...
        println(f)
        println(f, "─"^78)
        println(f, "  ТАБЛИЦА 2: РЕЗУЛЬТАТЫ ТЕСТОВ ($(result.tests_total) тестов RMT)")
        println(f, "─"^78)
        ...
        rows3 = [
            ...
            [3, "K(t) ≈ K_GUE(t) (теория)", "corr(GUE) > 0.8", result.K_match_GUE ? "✓ PASS" : "✗ FAIL"],
            ...
            [7, "K(t) коррелирует с K(ζ) (эмпирика)", "corr(ζ) > 0.7", result.K_match_zeta ? "✓ PASS" : "✗ FAIL"],
            [8, "Байт-инвариантность + RNG энтропия", "Δ⟨r⟩(256 ур.)<0.02 & H>7.9 бит & |χ²z|<3", result.byte_match ? "✓ PASS" : "✗ FAIL"]
        ]
        println(f, format_table(headers3, rows3))
        println(f)
        n_pass = sum([r[4] == "✓ PASS" ? 1 : 0 for r in rows3])
        println(f, "  Итого: $n_pass / $(result.tests_total) тестов пройдено")
        println(f)
        println(f, "═"^78)
    end
...
function save_json_report(result::MetricsResult, all_results::Vector{MetricsResult},
    ...
    data = Dict(
        ...
        "best_result" => Dict(
            ...
            "K_match_GUE" => result.K_match_GUE, "K_match_zeta" => result.K_match_zeta,
            "byte_quant_diff" => result.byte_quant_diff, "byte_quant_robust" => result.byte_quant_robust,
            "rng_entropy_bits" => result.rng_entropy_bits, "rng_chi2_z" => result.rng_chi2_z,
            "rng_uniform_ok" => result.rng_uniform_ok, "byte_match" => result.byte_match,
...
function plot_6panel_comparison(eigs_ab::AbstractVector{<:Real},
    ...
    Plots.with() do
        ...
        t_values = collect(0.0:0.02:2.0)
        K_ab = compute_K_empirical(eigs_ab, t_values; method=:polynomial, smooth=true, smooth_window=3)
        K_zeta = compute_K_empirical(zeta_zeros, t_values; method=:polynomial, smooth=true, smooth_window=3)
        K_gue = [K_GUE_theory(t) for t in t_values]
...
function _final_run(cfg::RHConfig, alpha_opt::Float64, W_opt::Float64, sigma_opt::Float64,
    ...
    rows = [
        ...
        ["Байт Δ⟨r⟩(256ур.)", round(final_result.byte_quant_diff, digits=6), "→ 0"],
        ["RNG энтропия(бит)", round(final_result.rng_entropy_bits, digits=4), "→ 8.0"],
        ["Тестов", "$(final_result.tests_passed)/$(final_result.tests_total)", "$(final_result.tests_total)/$(final_result.tests_total)"]
    ]
    ...
    println("    3. K(t) ≈ K_GUE(t) (теория)       : $(final_result.K_match_GUE ? "✓ PASS" : "✗ FAIL")")
    ...
    println("    7. K(t) коррелирует с K(ζ) эмпирич.: $(final_result.K_match_zeta ? "✓ PASS" : "✗ FAIL")  (раньше эта проверка молча пряталась внутри теста 3 под чужим именем)")
    println("    8. Байт-инвариантность + RNG-энтропия: $(final_result.byte_match ? "✓ PASS" : "✗ FAIL")")
...
println("    cfg = preset_irrational(); run(cfg)     — иррациональное α (1/π)")
println("    cfg = preset_irrational(:phi); run(cfg)  — золотое сечение")
println("═"^78)
println("  Helpers:")
println("    list_presets()    — список всех пресетов")
println("    verify_high_T_universality() — GUE на индексах ~1e12/1e21/1e22 (Odlyzko)")
