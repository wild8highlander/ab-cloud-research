# `logs/` — Computation Log and Console Capture of Test 34
...
This `logs/` subfolder belongs to Test 34 of the reference run; its parent folder [`../README.md`](../README.md) carries the full test description, verdicts and explanation.

## Complete contents inventory

| File | Size | Description |
|---|---|---|
| `computation_log_A34L.txt` | 812 B · 8 lines | Timestamped computation log: every log_comp() entry emitted while the test ran, with wall-clock offsets. |
| `stdout_capture_A34L.txt` | 1.6 KB · 22 lines | Raw console capture: the suite's console output for this test, including lettered sub-check lines and the verdict block. |

## The first lines of the computation log (verbatim)

```text
FULL COMPUTATION LOG — test 34: DIRECT AB-cloud vs ζ (full loaded set)
...
- The same log and capture ship as raw provenance copies in [`../FINAL_REPORT/logs/`](../../FINAL_REPORT/logs/README.md) — specifically its `test_34_direct_vs_zeta/` subfolder.
- Both are embedded verbatim inside the test's own report files (`../report_34.md` and its HTML/PDF/DOCX siblings) — see [`../README.md`](../README.md), section 7–8.
