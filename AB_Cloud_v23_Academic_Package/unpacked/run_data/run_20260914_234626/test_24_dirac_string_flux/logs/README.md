# `logs/` — Computation Log and Console Capture of Test 24
...
This `logs/` subfolder belongs to Test 24 of the reference run; its parent folder [`../README.md`](../README.md) carries the full test description, verdicts and explanation.

## Complete contents inventory

| File | Size | Description |
|---|---|---|
| `computation_log_A24L.txt` | 642 B · 7 lines | Timestamped computation log: every log_comp() entry emitted while the test ran, with wall-clock offsets. |
| `stdout_capture_A24L.txt` | 1.4 KB · 17 lines | Raw console capture: the suite's console output for this test, including lettered sub-check lines and the verdict block. |

## The first lines of the computation log (verbatim)

```text
FULL COMPUTATION LOG — test 24: Dirac string flux verification
...
- The same log and capture ship as raw provenance copies in [`../FINAL_REPORT/logs/`](../../FINAL_REPORT/logs/README.md) — specifically its `test_24_dirac_string_flux/` subfolder.
- Both are embedded verbatim inside the test's own report files (`../report_24.md` and its HTML/PDF/DOCX siblings) — see [`../README.md`](../README.md), section 7–8.
