# `logs/test_34_direct_vs_zeta/` — Raw Log Copies of Test 34 (Provenance Set)

This folder is part of FINAL_REPORT's provenance layer: it preserves the **raw computation log and console capture of Test 34** exactly as the reference run wrote them. The reading copies — with the full test description, verdicts and excerpts — live in the test's own folder: [`../../test_34_direct_vs_zeta/`](../../test_34_direct_vs_zeta/README.md).
...
|---|---|---|
| `computation_log_XX.txt` | 812 B · 8 lines | Timestamped computation log: every log_comp() entry emitted while the test ran, with wall-clock offsets. |
