# `logs/test_21_gamma_phase/` — Raw Log Copies of Test 21 (Provenance Set)

This folder is part of FINAL_REPORT's provenance layer: it preserves the **raw computation log and console capture of Test 21** exactly as the reference run wrote them. The reading copies — with the full test description, verdicts and excerpts — live in the test's own folder: [`../../test_21_gamma_phase/`](../../test_21_gamma_phase/README.md).
...
|---|---|---|
| `computation_log_XX.txt` | 138 B · 4 lines | Timestamped computation log: every log_comp() entry emitted while the test ran, with wall-clock offsets. |
| `stdout_capture_XX.txt` | 1.2 KB · 14 lines | Raw console capture: the suite's console output for this test, including lettered sub-check lines and the verdict block. |
