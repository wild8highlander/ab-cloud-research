# `logs/test_23_fractal_factor/` — Raw Log Copies of Test 23 (Provenance Set)

This folder is part of FINAL_REPORT's provenance layer: it preserves the **raw computation log and console capture of Test 23** exactly as the reference run wrote them. The reading copies — with the full test description, verdicts and excerpts — live in the test's own folder: [`../../test_23_fractal_factor/`](../../test_23_fractal_factor/README.md).
...
|---|---|---|
| `computation_log_XX.txt` | 135 B · 4 lines | Timestamped computation log: every log_comp() entry emitted while the test ran, with wall-clock offsets. |
