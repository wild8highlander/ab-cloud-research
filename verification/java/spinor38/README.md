# Test 38 -- 64 spinor structures of the Klein quartic (Java port)

Java port of the validated C++ reference `verification/cpp/spinor38/spinor38.cpp`.

The program uses ONLY the frozen data files and a self-implemented cyclic
Jacobi eigenvalue algorithm (no BLAS/LAPACK, standard library only) to verify:
...
6 levels) looking for `verification/spinor64/data/spinor_classes.csv`:
    ...
    javac Spinor38.java && java Spinor38 /path/to/repo/root

Requires JDK 8 or newer (standard library only). `run.sh` compiles
`Spinor38.java` in place and runs it; the program exits 0 on PASS / 1 on FAIL.
...
Note: this Java port was authored in an environment without a JDK, so it was
...
| `Spinor38.java` | 7.0 KB | file |
| `run.sh` | 199 B | file |
| **Total (recursive)** | **3 files, 9.3 KB** | |

## ▶ Running — toolchain and entry point

- **Toolchain:** JDK ≥ 17 — standard library only
- **Run:** `bash run.sh   # or: javac Spinor38.java && java Spinor38 [repo-root]`
